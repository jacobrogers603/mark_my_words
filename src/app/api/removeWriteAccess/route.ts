      ...
      try {
        ...
        if (!note.readAccessList.includes(allowedEmail)) {
          return NextResponse.json({
            error:
              "User does not have read access, they need read access to have access at all, if you want to remove full access use the removeFromAccessList route.",
          });
        }

        if (!note.writeAccessList.includes(allowedEmail)) {
          return NextResponse.json({
            error: "User already does not have write access",
          });
        }

        const updatedWriteAccessList = note.writeAccessList.filter(
